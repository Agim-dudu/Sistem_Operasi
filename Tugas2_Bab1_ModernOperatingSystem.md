...
<p> &nbsp &nbsp &nbsp Besides Windows, the other main example we will use throughout this book is
...
<p> &nbsp &nbsp &nbsp But even this level is much too low for most applications. For this reason, all
...
deal with the messy details of how the hardware actually works.</p>

<p> &nbsp &nbsp &nbsp This abstraction is the key to managing all this complexity. Good abstractions
...
abstractions. They are one of the keys to understanding operating systems.</p>

<p> &nbsp &nbsp &nbsp This point is so important that it is worth repeating in different words. With all
...
<p> &nbsp &nbsp &nbsp It should be noted that the operating system’s real customers are the application programs (via the application programmers, of course). They are the ones
...
<p> &nbsp &nbsp &nbsp In this book, we will study the abstractions provided to application programs in
...
<p> &nbsp &nbsp &nbsp One of the designers of OS/360, Fred Brooks, subsequently wrote a witty and
...
<p> &nbsp &nbsp &nbsp This desire for quick response time paved the way for <b>timesharing</b>, a variant
...
<p> &nbsp &nbsp &nbsp By the end of the 20th century, the concept of a computer utility had fizzled
...
<p> &nbsp &nbsp &nbsp The desire for a free production (as opposed to educational) version of MINIX
...
<p> &nbsp &nbsp &nbsp Next comes the cache memory, which is mostly controlled by the hardware. Main memory is divided up into <b>cache lines</b>, typically 64 bytes, with addresses 0 to 63 in cache line 0, 64 to 127 in cache line 1, and so on. The most heavily used
...
<p align="center"><b>Figure 1-11</b>. (a) The steps in starting an I/O device and getting an interrupt. (b)
...
<p> &nbsp &nbsp &nbsp The <b>SCSI (Small Computer System Interface)</b> bus is a high-performance bus intended for fast disks, scanners, and other devices needing considerable bandwidth. Nowadays, we find them mostly in servers and workstations. They can run
...
<p> &nbsp &nbsp &nbsp When any user logs in, a shell is started up. The shell has the terminal as standard input and standard output. It starts out by typing the <b>prompt</b>, a character such as a dollar sign, which tells the user that the shell is waiting to accept a command. If the user now types</p>
...
<p> &nbsp &nbsp &nbsp The user can specify that standard output be redirected to a file, for example,</p>
...
<p> &nbsp &nbsp &nbsp The output of one program can be used as the input for another program by connecting them with a pipe. Thus</p>
...
<p> &nbsp &nbsp &nbsp If a user puts an ampersand after a command, the shell does not wait for it to complete. Instead it just gives a prompt immediately. Consequently,</p>
...
<p> &nbsp &nbsp &nbsp Now let us look at operating systems. The first mainframes initially had no protection hardware and no support for multiprogramming, so they ran simple operating systems that handled one manually loaded program at a time. Later they acquired the hardware and operating system support to handle multiple programs at once, and then full timesharing capabilities.</P>
...
<p> &nbsp &nbsp &nbsp Let us consider the case of a command such as</p>
...
<p> &nbsp &nbsp &nbsp The second parameter, <i>argv</i>, is a pointer to an array. Element <i>i</I> of that array is a pointer to the _i_th string on the command line. In our example, <i>argv</i>[0] would point to the string ‘‘cp’’, <i>argv</i>[1] would point to the string ‘‘file1’’, and <I>argv</I>[2] would point to the string ‘‘file2’’.

<p> &nbsp &nbsp &nbsp The third parameter of <i>main</i>, <i>envp</i>, is a pointer to the environment, an array of strings containing assignments of the form <i>name = value</i> used to pass information such as the terminal type and home directory name to programs. There are library procedures that programs can call to get the environment variables, which are often used to customize how a user wants to perform certain tasks (e.g., the default printer to use). In Fig. 1-19, no environment is passed to the child, so the third parameter of <i>execve</i> is a zero.</p>
...
<p>The file descriptor returned can then be used for reading or writing. Afterward, the
file can be closed by close, which makes the file descriptor available for reuse on a
subsequent open.</p>

</p> &nbsp &nbsp &nbsp The most heavily used calls are undoubtedly read and wr ite. We saw read earlier. Wr ite has the same parameters.</P>

</p> &nbsp &nbsp &nbsp Although most programs read and write files sequentially, for some applications programs need to be able to access any part of a file at random. Associated
...
read or wr ite can begin anywhere in the file.</P>

</p> &nbsp &nbsp &nbsp Lseek has three parameters: the first is the file descriptor for the file, the second is a file position, and the third tells whether the file position is relative to the
...
file, subsequent changes made to one copy do not affect the others.</p>

</p> &nbsp &nbsp &nbsp To see how link works, consider the situation of Fig. 1-21(a). Here are two
users,<i> ast</i> and <i>jim</i>, each having his own directory with some files. If ast now executes a program containing the system call</p>

</p> &nbsp &nbsp &nbsp link("/usr/jim/memo", "/usr/ast/note");</p>

<p>the file memo in jim’s directory is now entered into ast’s directory under the name
...
<p align="center"><b>Figure 1-21</b>. (a) Two directories before linking /usr/jim/memo to ast’s directory.
(b) The same directories after linking.</p>

</p> &nbsp &nbsp &nbsp Understanding how link works will probably make it clearer what it does.
Every file in UNIX has a unique number, its i-number, that identifies it. This
i-number is an index into a table of<b> i-nodes</b>, one per file, telling who owns the file,
...
complicated structure is needed to support long file names, but conceptually a directory is still a set of (i-number, ASCII name) pairs. In Fig. 1-21, <i>mail</i> has i-number 16, and so on. What link does is simply create a brand new directory entry with
...
</p> &nbsp &nbsp &nbsp As we have mentioned earlier, the mount system call allows two file systems to
...
<p>where the first parameter is the name of a block special file for USB drive 0, the
...
<p> &nbsp &nbsp &nbsp an open on the file <i>xyz</i> will open <i>/usr/ast/test/xyz</i>. The concept of a working directory eliminates the need for typing (long) absolute path names all the time.</p>

<p> &nbsp &nbsp &nbsp In UNIX every file has a mode used for protection. The mode includes the
read-write-execute bits for the owner, group, and others. The chmod system call
makes it possible to change the mode of a file. For example, to make a file readonly by everyone except the owner, one could execute</p>
...
<p> &nbsp &nbsp &nbsp POSIX defines a number of procedures for dealing with time. For example,
...
<p> &nbsp &nbsp &nbsp Of course, Windows also has system calls. With UNIX, there is almost a oneto-one relationship between the system calls (e.g., read) and the library procedures
(<i>e.g., read</i>) used to invoke the system calls. In other words, for each system call,
there is roughly one library procedure that is called to invoke it, as indicated in
Fig. 1-17. Furthermore, POSIX has only about 100 procedure calls.</p>

<p> &nbsp &nbsp &nbsp With Windows, the situation is radically different. To start with, the library
calls and the actual system calls are highly decoupled. Microsoft has defined a set
of procedures called the <b>Win32 API (Application Programming Interface</b>) that
...
traps to the kernel).</p>

<p> &nbsp &nbsp &nbsp The Win32 API has a huge number of calls for managing windows, geometric
...
consult one of the many books on the subject (e.g., Hart, 1997; Rector and Newcomer, 1997; and Simon, 1997).</p>

<p> &nbsp &nbsp &nbsp Even introducing all the Win32 API calls here is out of the question, so we will
restrict ourselves to those calls that roughly correspond to the functionality of the
UNIX calls listed in Fig. 1-18. These are listed in Fig. 1-23.</p>

<p> &nbsp &nbsp &nbsp Let us now briefly go through the list of Fig. 1-23. CreateProcess creates a
...
specified process to exit, which is done using ExitProcess.</p>

<p> &nbsp &nbsp &nbsp The next six calls operate on files and are functionally similar to their UNIX
...
GetFileAttr ibutesEx calls set the file position and get some of the file attributes.</p>

<p> &nbsp &nbsp &nbsp Windows has directories and they are created with CreateDirector y and
RemoveDirector y API calls, respectively. There is also a notion of a current directory, set by SetCurrentDirector y. The current time of day is acquired using GetLocalTime.</p>
...
<p align="center"><b>Figure 1-23</b>. The Win32 API calls that roughly correspond to the UNIX calls of
Fig. 1-18. It is worth emphasizing that Windows has a very large number of other system calls, most of which do not correspond to anything in UNIX.</p>
...
we will discuss here are monolithic systems, layered systems, microkernels, client-server systems, virtual machines, and exokernels.</p>

<p align="center"> OPERATING SYSTEM STRUCTURE </p>

<h3>1.7.1 Monolithic Systems</h3>

<p> &nbsp &nbsp &nbsp By far the most common organization, in the monolithic approach the entire
...
procedures will take down the entire operating system.</p>

<p> &nbsp &nbsp &nbsp To construct the actual object program of the operating system when this approach is used, one first compiles all the individual procedures (or the files containing the procedures) and then binds them all together into a single executable
...
<p> &nbsp &nbsp &nbsp Even in monolithic systems, however, it is possible to have some structure. The
...
to the procedure that carries out system call k (step 7 in Fig. 1-17).</p>

<p> &nbsp &nbsp &nbsp This organization suggests a basic structure for the operating system:</p>

<p> &nbsp &nbsp &nbsp 1. A main program that invokes the requested service procedure.</p>
<p> &nbsp &nbsp &nbsp 2. A set of service procedures that carry out the system calls.</p>
<p> &nbsp &nbsp &nbsp 3. A set of utility procedures that help the service procedures.</p>

<p>In this model, for each system call there is one service procedure that takes care of
...
procedures into three layers is shown in Fig. 1-24.</p>

<p> &nbsp &nbsp &nbsp In addition to the core operating system that is loaded when the computer is
...
The THE system was a simple batch system for a Dutch computer, the Electrologica X8, which had 32K of 27-bit words (bits were expensive back then).</p>

<p> &nbsp &nbsp &nbsp The system had six layers, as shown in Fig. 1-25. Layer 0 dealt with allocation
...
<p> &nbsp &nbsp &nbsp Layer 1 did the memory management. It allocated space for processes in main
...
<p> &nbsp &nbsp &nbsp Layer 2 handled communication between each process and the operator console (that is, the user). On top of this layer each process effectively had its own operator console. Layer 3 took care of managing the I/O devices and buffering the
...
process was located in layer 5.</p>

<p> &nbsp &nbsp &nbsp A further generalization of the layering concept was present in the MULTICS
...
<p> &nbsp &nbsp &nbsp Whereas the THE layering scheme was really only a design aid, because all the
...
<p> &nbsp &nbsp &nbsp Various researchers have repeatedly studied the number of bugs per 1000 lines
...
<p> &nbsp &nbsp &nbsp The basic idea behind the microkernel design is to achieve high reliability by
...
reference an invalid memory address and bring the system to a grinding halt instantly.</p>

<p> &nbsp &nbsp &nbsp Many microkernels have been implemented and deployed for decades (Haertig
...
system freely available at <i>www.minix3.org</i> (Giuffrida et al., 2012; Giuffrida et al.,
2013; Herder et al., 2006; Herder et al., 2009; and Hruby et al., 2013).</p>

<p> &nbsp &nbsp &nbsp The MINIX 3 microkernel is only about 12,000 lines of C and some 1400 lines
...
closely with it. The other device drivers run as separate user processes.</p>
...
<p>the kernel to do the write. This approach means that the kernel can check to see
...
<p> &nbsp &nbsp &nbsp Above the drivers is another user-mode layer containing the servers, which do
...
do a read sends a message to one of the file servers telling it what to read.</p>

<p> &nbsp &nbsp &nbsp One interesting server is the <b>reincarnation server</b>, whose job is to check if the
...
the system is self healing and can achieve high reliability.</p>

<p> &nbsp &nbsp &nbsp The system has many restrictions limiting the power of each process. As mentioned, drivers can touch only authorized I/O ports, but access to kernel calls is also
...
more, thus greatly limiting the damage a buggy component can do.</p>

<p> &nbsp &nbsp &nbsp An idea somewhat related to having a minimal kernel is to put the <b>mechanism</b>
...
layer is a microkernel, but that is not required. The essence is the presence of client processes and server processes.</p>

<p> &nbsp &nbsp &nbsp Communication between clients and servers is often by message passing. To
...
<p> &nbsp &nbsp &nbsp An obvious generalization of this idea is to have the clients and servers run on
...
<p> &nbsp &nbsp &nbsp Increasingly many systems involve users at their home PCs as clients and large
...
a group at IBM’s Scientific Center in Cambridge, Massachusetts, produced a radically different system that IBM eventually accepted as a product. A linear descendant of it, called <b>z/VM</b>, is now widely used on IBM’s current mainframes, the
...
separate these two functions.</p>

<p> &nbsp &nbsp &nbsp The heart of the system, known as the <b>virtual machine monitor</b>, runs on the
...
<p align="center">Figure 1-28. The structure of VM/370 with CMS.</p>
...
users. The latter was popular with programmers.</p>

<p> &nbsp &nbsp &nbsp When a CMS program executed a system call, the call was trapped to the operating system in its own virtual machine, not to VM/370, just as it would be were it
...
carry out the call. These I/O instructions were trapped by VM/370, which then  them as part of its simulation of the real hardware. By completely separating the functions of multiprogramming and providing an extended machine, each
of the pieces could be much simpler, more flexible, and much easier to maintain.</p>
...
combined to make it a hot topic.</p>

<p> &nbsp &nbsp &nbsp First the needs. Many companies have traditionally run their mail servers, Web
...
same machine without having a crash of one server bring down the rest.</p>

<p> &nbsp &nbsp &nbsp Virtualization is also popular in the Web hosting world. Without virtualization,
Web hosting customers are forced to choose between <b>shared hosting</b> (which just
...
machine supports many virtual machines at the same time).</p>

<p> &nbsp &nbsp &nbsp Another use of virtualization is for end users who want to be able to run two or
...
<p> &nbsp &nbsp &nbsp While no one disputes the attractiveness of virtual machines today, the problem
...
a performance loss of one to two orders of magnitude, they were not useful for serious work.</p>

<p> &nbsp &nbsp &nbsp This situation changed as a result of several academic research projects in the
...
and Hyper-V (by Microsoft).</p>

<p> &nbsp &nbsp &nbsp Some of these early research projects improved the performance over interpreters like <i>Bochs</i> by translating blocks of code on the fly, storing them in an internal cache, and then reusing them if they were executed again. This improved the
...
(and have many other improvements as well). They are called <b>type 2 hypervisors</b>
...
<p> &nbsp &nbsp &nbsp In practice, the real distinction between a type 1 hypervisor and a type 2 hypervisor is that a type 2 makes uses of a <b>host operating system</b> and its file system to
create processes, store files, and so on. A type 1 hypervisor has no underlying support and must perform all these functions itself.</p>

<p> &nbsp &nbsp &nbsp After a type 2 hypervisor is started, it reads the installation CD-ROM (or CDROM image file) for the chosen <b>guest operating system</b> and installs the guest OS
...
store files on. They must manage their own storage on a raw disk partition.</p>

<p> &nbsp &nbsp &nbsp When the guest operating system is booted, it does the same thing it does on
...
running on the bare metal even though that is not the case here.</p>
...
might get blocks 1024 to 2047, and so on.</p>

<p> &nbsp &nbsp &nbsp At the bottom layer, running in kernel mode, is a program called the <b>exokernel</b>
...
using only the resources it has asked for and been allocated.</p>

<p> &nbsp &nbsp &nbsp The advantage of the exokernel scheme is that it saves a layer of mapping. In
...
there are many similarities between the two. <b>Python</b> is somewhat different, but still
...
<p>which declare <i>c1</i> and <i>c2</i> to be character variables and <i>p</i> to be a variable that points
to (i.e., contains the address of) a character. The first assignment stores the ASCII
code for the character ‘‘c’’ in the variable <i>c1</i>. The second one assigns the address
of <i>c1</i> to the pointer variable <i>p</i>. The third one assigns the contents of the variable
pointed to by <i>p</i> to the variable <i>c2</i>, so after these statements are executed, <i>c2</i> also
...
<p> &nbsp &nbsp &nbsp Some things that C does not have include built-in strings, threads, packages,
...
released by the programmer, usually with the library functions <i>malloc</i> and <i>free</i>. It
...
along with some .<i>h</i> header files that contain declarations and definitions used by
one or more code files. Header files can also include simple <b>macros</b>, such as</p>

<p> &nbsp &nbsp &nbsp #define BUFFER SIZE 4096</p>

<p>which allows the programmer to name constants, so that when BUFFER SIZE is
used in the code, it is replaced during compilation by the number 4096. Good C
programming practice is to name every constant except 0, 1, and −1, and sometimes even them. Macros can have parameters, such as</p>
...
which compiles into a call to the function <i>intel_int_ack</i> if the macro X86 is defined
...
like Java byte code or Python byte code in the C world.</p>

<p> &nbsp &nbsp &nbsp The first pass of the C compiler is called the <b>C preprocessor</b>. As it reads each
.<i>c</i> file, every time it hits a <i>#include</i> directive, it goes and gets the header file named
...
they were physically included.</p>

<p> &nbsp &nbsp &nbsp Since operating systems are very large (fiv e million lines of code is not
...
which object files depend on which header files is completely unmanageable without help.</p>

<p> &nbsp &nbsp &nbsp Fortunately, computers are very good at precisely this sort of thing. On UNIX
systems, there is a program called <i>make</i> (with numerous variants such as <,>gmake,
pmake, etc.</i>) that reads the <i>Makefile</i>, which tells it which files are dependent on
...
to 30 years from idea to impact.</p>

<p> &nbsp &nbsp &nbsp For example, when President Eisenhower set up the Dept. of Defense’s Advanced Research Projects Agency (ARPA) in 1958, he was trying to keep the
...
lab in Geneva and Marc Andreesen wrote a graphical browser for it at the University of Illinois. All of a sudden the Internet was full of twittering teenagers. President Eisenhower is probably rolling over in his grave.</p>

<p> &nbsp &nbsp &nbsp Research in operating systems has also led to dramatic changes in practical
...
<p> &nbsp &nbsp &nbsp USENIX &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp&nbsp &nbsp &nbsp &nbsp &nbsp &nbsp http://www.usenix.org</p>

<p> &nbsp &nbsp &nbsp Virtually all operating systems researchers realize that current operating systems are massive, inflexible, unreliable, insecure, and loaded with bugs, certain
ones more than others (<i>names withheld here to protect the guilty</i>). Consequently,
...
of how interprocess communication works and how to avoid some of the pitfalls.</p>

<p> &nbsp &nbsp &nbsp In Chap. 3 we will study address spaces and their adjunct, memory management, in detail. The important topic of virtual memory will be examined, along
with closely related concepts such as paging and segmentation.</p>

<p> &nbsp &nbsp &nbsp Then, in Chap. 4, we come to the all-important topic of file systems. To a considerable extent, what the user sees is largely the file system. We will look at both
the file-system interface and the file-system implementation.</p>

<p> &nbsp &nbsp &nbsp Input/Output is covered in Chap. 5. The concepts of device independence and
device dependence will be looked at. Several important devices, including disks,
keyboards, and displays, will be used as examples.</p>

<p> &nbsp &nbsp &nbsp Chapter 6 is about deadlocks. We briefly showed what deadlocks are in this
chapter, but there is much more to say. Ways to prevent or avoid them are discussed.</p>

<p> &nbsp &nbsp &nbsp At this point we will have completed our study of the basic principles of single-CPU operating systems. However, there is more to say, especially about advanced topics. In Chap. 7, we examine virtualization. We discuss both the principles, and some of the existing virtualization solutions in detail. Since virtualization is heavily used in cloud computing, we will also gaze at existing clouds.
Another advanced topic is multiprocessor systems, including multicores, parallel
computers, and distributed systems. These subjects are covered in Chap. 8.</p>

<p> &nbsp &nbsp &nbsp A hugely important subject is operating system security, which is covered in
Chap 9. Among the topics discussed in this chapter are threats (e.g., viruses and
worms), protection mechanisms, and security models.</p>
...
<p> &nbsp &nbsp &nbsp It is also worth pointing out that, in common industry practice, the units for
...
<p> &nbsp &nbsp &nbsp Operating systems have a long history, starting from the days when they replaced the operator, to modern multiprogramming systems. Highlights include
early batch systems, multiprogramming systems, and personal computer systems.</p>

<p> &nbsp &nbsp &nbsp Since operating systems interact closely with the hardware, some knowledge
of computer hardware is useful to understanding them. Computers are built up of
processors, memories, and I/O devices. These parts are connected by buses.</p>
...
group is for directory management. The fourth group contains miscellaneous calls.</p>
...
8. One reason GUIs were initially slow to be adopted was the cost of the hardware needed to support them. How much video RAM is needed to support a 25-line × 80-row
character monochrome text screen? How much for a 1200 × 900-pixel 24-bit color bitmap? What was the cost of this RAM at 1980 prices ($5/KB)? How much is it now?
...
Suppose three programs, <i>P0</i>, <i>P1</i>, and <i>P2</i>, are started with run times of 5, 10 and 20
...
call return any value in <i>count</i> other than <i>nbytes</i>? If so, why?

23. A file whose file descriptor is <i>fd</i> contains the following sequence of bytes: 3, 1, 4, 1, 5,
...
26. In the example given in Fig. 1-17, the library procedure is called <i>read</i> and the system
...
tool like the UNIX <i>od</i> program. (<i>Hint</i>: How you do this will depend upon what the OS
